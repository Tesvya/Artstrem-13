...
public class DashboardClient extends JFrame implements ActionListener, StoreRequests, ItemListener
{
    ...
    {
        new ImageIcon("artistlogo.png"),
        new ImageIcon("pending-removebg-preview.png"),
        new ImageIcon("accepted-removebg-preview.png"),
        new ImageIcon("profile-removebg-preview.png")
    };
   ...
   // private JComboBox <String> typeOfArtists = new JComboBox<>(artists);
    ...
    public DashboardClient(String name)
    {
        ...
        for(int i = 0; i < panelBorder.length; ++i)
        {
            ...
            {
                ImageIcon profileOfClient = new ImageIcon("profile.jpg");
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        switch (command) {
            ...
            case "digital":
                ...
                DigitalArtists digi = new DigitalArtists();
                if(digi.getArraySize() == 0)
                    JOptionPane.showMessageDialog(null, "No Digital Artists Available");
                else
                    card.show(panelBorder[1], command);
                break;
             case "charcoal":
                CharcoalArtists charcoal = new CharcoalArtists();
                if(charcoal.getArraySize() == 0)
                    JOptionPane.showMessageDialog(null, "No Charcoal Artists Available");
                else
                    card.show(panelBorder[1], command);
                break;
            case "landscape":
                LandscapeArtists land = new LandscapeArtists();
                if(land.getArraySize() == 0)
                    JOptionPane.showMessageDialog(null, "No Landscape Artists Available");
                else
                   card.show(panelBorder[1], command);
                break;
            case "portrait":
                PortraitArtists port = new PortraitArtists();
                if(port.getArraySize() == 0)
                      JOptionPane.showMessageDialog(null, "No Landscape Artists Available");
                else
                card.show(panelBorder[1], command);
                break;        
